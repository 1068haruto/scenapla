...
紙に計算を書き出し、試行錯誤を重ねることで解消できましたが、そのプロセスは煩雑で時間がかかるものでした。
この経験から、手軽に状況や見通しを把握でき、計画をアシストできるサービスがあれば、
同じように悩む人の役に立つのではないかと考え、本サービスを作成しました。
...
本サービスは、この時期に具体的な将来のイメージと行動指針を示し、早期修正を促すことで、今後の方針や計画のサポートを行います。
### 2. 転職や資産運用を検討し始める方  
...
## サービスの差別化ポイント・推しポイント  
- **柔軟なシミュレーション**  
  ライフプランに合わせて支出項目を追加できるため、ユーザーに状況に合う入力が可能  
- **シミュレーション × ライフプラン管理**  
  登録したライフイベントとメモを管理でき、シミュレーションと考えの整理を１ストップで行える  
- **将来からの逆算シミュレーション**  
